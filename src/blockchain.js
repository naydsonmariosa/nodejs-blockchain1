...
class Blockchain {
    ...
    requestMessageOwnershipVerification(address) {
        return new Promise((resolve) => {
            let time = new Date().getTime().toString().slice(0,-3).toString();
            let message = `${address}:${time}:starRegistry`
            resolve(message)
            reject('Error on requestMessageOwnershipVerification');
        });
    }
    ...
    submitStar(address, message, signature, star) {
        let self = this;
        return new Promise(async (resolve, reject) => {
            let messageTime = parseInt(message.toString().split(':')[1]);
    ...
    getStarsByWalletAddress (address) {
        ...
        return new Promise((resolve, reject) => {
            ...
            .filter(b => b.height > 0)
            .forEach(b => {
                let body = JSON.parse(Buffer.from(b.body, 'hex').toString());
            ...
            resolve(stars);
            reject('Error on getStarsByWalletAddress()')
        });
    }
    ...
    async validateChain() {
        ...
        return new Promise(async (resolve, reject) => {
            self.chain.forEach(block => {
                if (!block.validate()) {
                    errorLog.push(`Block Error: ${block.toString()}`)
                }
            });
            resolve(errorLog);
